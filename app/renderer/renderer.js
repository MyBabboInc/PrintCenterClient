...
async function loadPrinters() {
    try {
        ...
        // Listen for tray changes to apply profile presets
        traySelect.addEventListener('change', async () => {
            await applyPrinterProfile();
        });
    } catch (e) {
...
async function populateTrays() {
    ...
    try {
        ...
        // Apply printer profile presets after populating trays
        await applyPrinterProfile();
...
/**
 * Apply printer profile presets based on selected printer and tray
 */
async function applyPrinterProfile() {
    const printerName = printerSelect.value;
    const trayName = traySelect.value;

    if (!printerName || !window.profileLoader) {
        return;
    }

    try {
        // Load printer profiles
        const profilesData = await window.profileLoader.loadPrinterProfiles();
        const profiles = profilesData.printerProfiles || {};

        // Match printer to profile
        const matchedProfileKey = window.profileLoader.matchPrinterProfile(printerName, profiles);


        if (!matchedProfileKey) {
            console.log('No profile found for printer:', printerName);
        ...
        console.log(`Applying profile: ${matchedProfileKey}`);


        if (trayName && trayName !== '') {
            const preset = window.profileLoader.getTrayPreset(matchedProfileKey, trayName, profiles);

            if (preset) {
                const productKey = document.getElementById('product-select').value;
                const merged = window.profileLoader.mergeWithUserOverrides(preset, printerName, trayName, productKey);

                console.log('Applying tray preset:', merged);

                if (merged.duplex) {
                    const duplexSelect = document.getElementById('duplex-select');
                    duplexSelect.value = merged.duplex;
                }

                if (merged.mediaType) {
                    const mediaSelect = document.getElementById('media-select');
                    const option = Array.from(mediaSelect.options).find(
                        opt => opt.value === merged.mediaType || opt.textContent === merged.mediaType
                    );
                    if (option) {
                        mediaSelect.value = option.value;
                    } else {
                        console.warn(`Media type "${merged.mediaType}" not found in dropdown`);
                    }
                }

                if (merged.xShiftMM !== undefined) {
                    offsetX.value = merged.xShiftMM;
                    valXInput.value = merged.xShiftMM;
                }
                if (merged.yShiftMM !== undefined) {
                    offsetY.value = merged.yShiftMM;
                    valYInput.value = merged.yShiftMM;
                }
                ...
                console.log('No tray preset found for:', trayName);
                rotationSelect.disabled = false;
                window.activeProfile = null;
            }
        } else {
            console.log('No tray selected yet');
            rotationSelect.disabled = false;
            window.activeProfile = null;
        }

    } catch (err) {
        console.error('Failed to apply printer profile:', err);
        rotationSelect.disabled = false;
        window.activeProfile = null;
    }
}
function setupEventListeners() {
    ...
    if (!btnSaveDefaults) {
        ...
        btnSaveDefaults.addEventListener('click', async () => {
            ...
            if (!printerName || !trayName) {
                alert('Please select a printer and tray first.');
                return;
            }

            // Gather current settings as override
            const override = {
            ...
            // Save using profile loader
            if (window.profileLoader) {
                window.profileLoader.saveUserOverride(printerName, trayName, productKey, override);
                alert(`Settings saved for ${printerName} - ${trayName}${productKey ? ' - ' + productKey : ''}`);
                clearSettingsChanged();
            } else {
                console.error('Profile loader not available');
                alert('Failed to save settings - profile loader not initialized');
            }
        });
    }
...
async function printCurrentPdf() {
    ...
    if (window.profileLoader && currentPageSize.width > 0 && currentPageSize.height > 0) {
        ...
        if (isMultiUp && settings.duplex !== 'long' && settings.duplex !== '') {
            console.warn('Multi-up card format detected, forcing long-edge duplex');
            settings.duplex = 'long';
            // Update UI to reflect forced setting
            document.getElementById('duplex-select').value = 'long';
        }
    }
